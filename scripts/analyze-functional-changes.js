...
function analyzeFunctionalChanges(before, after, moduleName) {
  ...
  const commandChanges = analyzeCommands(
    beforeVersion.commands || [],
    afterVersion.commands || [],
  );
  const hookChanges = analyzeHooks(
    beforeVersion.hooks || [],
    afterVersion.hooks || [],
  );
  const cronChanges = analyzeCronJobs(
    beforeVersion.cronJobs || [],
    afterVersion.cronJobs || [],
  );
  const functionChanges = analyzeFunctions(
    beforeVersion.functions || [],
    afterVersion.functions || [],
  );
  const permissionChanges = analyzePermissions(
    beforeVersion.permissions || [],
    afterVersion.permissions || [],
  );
  const configChanges = analyzeConfig(
    beforeVersion.configSchema,
    afterVersion.configSchema,
  );
...
function generateDescription(allChanges) {
  ...
  if (added.length > 0) {
    ...
    Object.entries(byType).forEach(([type, items]) => {
      if (type === 'command') {
        parts.push(
          `Added ${items.length} command${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'hook') {
        parts.push(
          `Added ${items.length} hook${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\` (${i.eventType})`).join(', ')}`,
        );
      } else if (type === 'config') {
        parts.push(
          `New configuration option${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'cronjob') {
        parts.push(
          `Added ${items.length} scheduled task${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'function') {
        parts.push(
          `Added ${items.length} function${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'permission') {
        parts.push(
          `New permission${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      }
    });
  }
  ...
  if (removed.length > 0) {
    ...
    Object.entries(byType).forEach(([type, items]) => {
      if (type === 'command') {
        parts.push(
          `Removed ${items.length} command${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'hook') {
        parts.push(
          `Removed ${items.length} hook${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      } else if (type === 'config') {
        parts.push(
          `Removed configuration option${items.length > 1 ? 's' : ''}: ${items.map((i) => `\`${i.name}\``).join(', ')}`,
        );
      }
    });
  }
  ...
  if (updated.length > 0) {
    ...
    Object.entries(byType).forEach(([type, items]) => {
      if (type === 'version') {
        ...
        parts.push(
          `Updated ${items.length} command implementation${items.length > 1 ? 's' : ''}`,
        );
      } else if (type === 'hook') {
        parts.push(
