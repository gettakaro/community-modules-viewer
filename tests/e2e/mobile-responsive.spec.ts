...
test.describe('Mobile Responsiveness', () => {
  test.describe('Mobile Device Tests', () => {
    ...
    test('sidebar is hidden by default on mobile homepage', async ({ page }) => {
      await page.goto('/');
    ...
    test.skip('hamburger menu opens and closes on mobile module pages', async ({ page }) => {
    ...
    test.skip('mobile navigation closes sidebar after module selection', async ({ page }) => {
      ...
      const differentModuleLink = page.locator('[data-testid="module-link"]').filter({ hasText: 'economyUtils' }).first();
    ...
    test('mobile overlay blocks interaction with main content', async ({ page }) => {
      // Navigate to a module page
      await page.goto('/module/geoBlock');
  ...
  test.describe('Responsive Breakpoint Tests', () => {
    ...
    for (const breakpoint of breakpoints) {
      test(`layout adapts correctly at ${breakpoint.name} (${breakpoint.width}px)`, async ({ page }) => {
  ...
  test.describe('Touch Interactions', () => {
    ...
    test('all interactive elements work with click on mobile', async ({ page }) => {
      await page.goto('/module/geoBlock');

      // Test collapsible code toggle with click (instead of tap)
      const codeToggle = page.locator('button:has-text("Show"), button:has-text("Hide")').first();
      ...
      if (await exportButton.isVisible()) {
        ...
        await expect(page.locator('button:has-text("Export Current Version")')).toBeVisible();
      }
    });
  });
    ...
    test.skip('module details stack properly on mobile', async ({ page }) => {
      ...
      if (count > 1) {
        ...
        if (firstBox && secondBox) {
          expect(secondBox.y).toBeGreaterThan(firstBox.y + firstBox.height - 10); // Allow small overlap
        }
      }
    });
